...
interface DataRecordsDao : InsertableDataRecord {
    ...
    suspend fun byDataBlockId(dataBlockId: EID): List<DataRecord>
}
...
private class DataRecordsDaoImpl(
    ...
    context(cx: Connection)
    override suspend fun byDataBlockId(dataBlockId: EID): List<DataRecord> =
        DataRecords.select("r", "WHERE r.data_block_id = ?", dataBlockId.paramAny)
}
