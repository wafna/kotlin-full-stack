...
internal fun createMetaDAO() = object : MetaDAO {
    ...
    override suspend fun getTable(schemaName: String, tableName: String) = log.selectLogged(
        """SELECT ${tableMarshaller.project("ts")}
        |FROM information_schema.tables ts
        |WHERE ts.table_schema = ?
        |  AND ts.table_name = ?
    ...
    override suspend fun listForeignKeys(schemaName: String, tableName: String) = log.selectLogged(
        foreignKeys(FKDirection.FROM),
        schemaName,
        tableName
    ) { foreignKeyMarshaller.read(it) }

    override suspend fun listForeignKeyRefs(schemaName: String, tableName: String) = log.selectLogged(
